...
| **guardrails** | Enforces safe agent behavior by blocking tool usage patterns that degrade session quality. Blocks `sleep` commands, blocks `EnterPlanMode` (agent must describe approach and wait for approval instead), and enforces cron interval/count limits. Configurable via `GUARDRAILS_MAX_CRONS` and `GUARDRAILS_MIN_CRON_MINUTES`. |
