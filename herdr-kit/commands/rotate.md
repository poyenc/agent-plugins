...
- **Do not alias the CLI binary itself** (e.g. `alias codex='codex
  --dangerously-bypass-approvals-and-sandbox'`) — `herdr agent start` types the relaunch command
  into that same aliased shell, so the alias's flags stack with whatever was captured on every
  rotation. Put such flags directly in the launch command instead of a shell alias.
