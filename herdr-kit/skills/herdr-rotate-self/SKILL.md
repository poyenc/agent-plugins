...
description: >
  ...
  DIFFERENT agent's pane, use herdr-rotate instead -- this skill only works on the
...
(see herdr-rotate's SKILL.md, "Known limitations": `finish` cannot target the calling
...
1. Write your own handoff document now -- if you have a handoff skill available, invoke
   ...
   next step; there is no ping/wait mechanism in this skill, unlike herdr-rotate's
...
2. Run:
   ...
   `.../herdr-kit/skills/herdr-rotate-self`). This validates the handoff file, resolves
...
- All of herdr-rotate's own known limitations apply verbatim here, since the daemon
