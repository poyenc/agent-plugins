...
| **[ssh-host-guard](hooks/scripts/ssh-host-guard.sh)** | Enforces a block/allow list for `ssh`/`scp`/`rsync` destinations, configured via `GUARDRAILS_HOST_BLOCK`/`GUARDRAILS_HOST_ALLOW` (comma-separated glob patterns; unset or empty by default, so the hook is inert until you configure it). Every bareword argument is resolved through `ssh -G` before checking, so an `~/.ssh/config` alias is caught by its real `HostName`, not just its literal name. A host matching `ALLOW` is always permitted (even if also matched by `BLOCK`); otherwise a host matching `BLOCK` is denied; a host matching neither is permitted (an empty `ALLOW` means no exceptions to `BLOCK`, not "allow everything" — that only happens when `BLOCK` itself is empty/unset, since there's then nothing to match). |
...
blind-scan hooks, and the host guard). Run one, or all available:
